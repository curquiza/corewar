...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/01/10 10:49:42 by curquiza          #+#    #+#             */
/*   Updated: 2019/01/10 13:47:57 by curquiza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
								...
								signed short index, int size)
{
	...
	if (size >= 2)
		rslt = (rslt << 8)
			| (t_byte)vm_mem[get_mem_index(proc, index + 1, DEF_ADDR)].value;
	if (size >= 3)
		rslt = (rslt << 8)
			| (t_byte)vm_mem[get_mem_index(proc, index + 2, DEF_ADDR)].value;
	if (size >= 4)
		rslt = (rslt << 8)
			| (t_byte)vm_mem[get_mem_index(proc, index + 3, DEF_ADDR)].value;
	return (rslt);
}
